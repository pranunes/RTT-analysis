...
# Constants
RAW_COUNTS_FOLDER <- 'C:\\Users\\Ana\\Documents\\Estágio\\Tanaka2014\\'
...
import_featureCounts <- function(counts_file, clinical_data, te_class = FALSE) {
  ...
  colnames(counts_df)[7:total_cols] <- sub("/mnt/comics-data/comics02/RTT_files/Tanaka2014/03_Uniquelly_Mapped/", "", colnames(counts_df)[7:total_cols])
  colnames(counts_df)[7:total_cols] <- sub("_trimmed", "", colnames(counts_df)[7:total_cols])
  ...
  colnames(counts_df)[-1] <- clinical_data[["Sample.Name"]][match(srr_names, clinical_data$Run)] #Colocar na tabela final, todos os srr presentes na clinical data
  if (te_class == TRUE) {
...
organize_clinical_data <- function(clinical_data) {
  clinical_data$mutation[clinical_data$treatment != 'no'] <- 'shMECP2'
  
  clinical_data <- clinical_data[c('Run', 'Sample.Name', 'mutation', 'cell_type', 'PASSAGE', 'source_name')]
  clinical_data$cell_type <- sub(" fibroblast", "", clinical_data$cell_type)
  
  # Get the mutations
  clinical_data <- clinical_data[clinical_data$Run == 'SRR1015689' | 
                                clinical_data$Run == 'SRR1015687' |
                                clinical_data$Run == 'SRR1015691' | 
                                clinical_data$Run == 'SRR1015690' | 
                                clinical_data$mutation == 'no',]
  
  clinical_data$Condition[clinical_data$mutation == 'no'] <- 'WT'
  clinical_data$Condition[clinical_data$mutation != 'no'] <- 'RTT'
...
clinical_data <- read.csv(paste0(RAW_COUNTS_FOLDER, 'SraRunTable_Tanaka2014.csv'))
clinical_data <- organize_clinical_data(clinical_data)
...
res_dea <- dea_deseq(te_counts, clinical_data, save_expression = TRUE, 'Expression_TE_DESeq.csv')
#Saves the result, porque é que guardo aqui novamente?
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_TE_DESeq.csv'), quote = FALSE)


# Process gene counts
gene_counts <- import_featureCounts(paste0(RAW_COUNTS_FOLDER, 'Counts_Genes_Unique.txt'), clinical_data)
res_dea <- dea_deseq(gene_counts, clinical_data, save_expression = TRUE, "Expression_Genes_DESeq.csv")
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_Genes_DESeq.csv'), quote = FALSE)
...
res_dea <- dea_deseq(te_class_counts, clinical_data, save_expression = TRUE, 'Expression_TEClass_DESeq.csv')
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_TEClass_DESeq.csv'), quote = FALSE)
