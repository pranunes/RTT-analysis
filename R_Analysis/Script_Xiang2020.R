...
# Constants
RAW_COUNTS_FOLDER <- 'C:\\Users\\Ana\\Documents\\Estágio\\Xiang2020\\'
...
import_featureCounts <- function(counts_file, clinical_data, te_class = FALSE) {
  ...
  colnames(counts_df)[7:total_cols] <- sub("/mnt/comics-data/comics02/RTT_files/Xiang2020/03_Uniquelly_Mapped/", "", colnames(counts_df)[7:total_cols])
...
dea_deseq <- function(counts_df, clinical_info, save_expression = TRUE, save_expression_name = "Expression.csv") {
  ...
  res <- results(dds, contrast = c("Condition", "MECP2_mutant", "WT"), alpha = 0.05)
  print(summary(res))
...
clinical_data <- read.csv(paste0(RAW_COUNTS_FOLDER, 'Clinical_Data_Xiang20.csv'))
