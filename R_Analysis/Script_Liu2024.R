...
# Constants
RAW_COUNTS_FOLDER <- 'C:\\Users\\Ana\\Documents\\Estágio\\Liu2024\\'
...
import_featureCounts <- function(counts_file, clinical_data, te_class = FALSE) {
  ...
  colnames(counts_df)[7:total_cols] <- sub("/mnt/comics-data/comics02/RTT_files/Liu2024/03_Uniquelly_Mapped/", "", colnames(counts_df)[7:total_cols])
  ...
  print(clinical_data[["Sample.Name"]][match(srr_names, clinical_data$Run)])
...
dea_deseq <- function(counts_df, clinical_info, save_expression = TRUE, save_expression_name = "Expression.csv") {
  ...
  dds <- DESeqDataSetFromMatrix(countData = counts_df, colData = clinical_info, design = ~ genotype )
  ...
  res <- results(dds, contrast = c("genotype", "MECP2.KO", "MECP2.WT"), alpha = 0.05)
  print(summary(res))
...
clinical_data <- read.csv(paste0(RAW_COUNTS_FOLDER, 'SraRunTable_Liu24.csv'))
print(clinical_data)

clinical_data <- clinical_data[c('Run', 'Sample.Name', 'genotype')]
...
res_dea <- dea_deseq(te_counts, clinical_data, save_expression = TRUE, 'Expression_TE_DESeq_KO.csv')
#Saves the result, porque é que guardo aqui novamente?
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_TE_DESeq_KO.csv'), quote = FALSE)


# Process gene counts
gene_counts <- import_featureCounts(paste0(RAW_COUNTS_FOLDER, 'Counts_Genes_Unique.txt'), clinical_data)
res_dea <- dea_deseq(gene_counts, clinical_data, save_expression = TRUE, "Expression_Genes_DESeq_KO.csv")
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_Genes_DESeq_KO.csv'), quote = FALSE)
...
res_dea <- dea_deseq(te_class_counts, clinical_data, save_expression = TRUE, 'Expression_TEClass_DESeq_KO.csv')
write.csv(res_dea, file = paste0(RAW_COUNTS_FOLDER, 'DEA_TEClass_DESeq_KO.csv'), quote = FALSE)
