setwd('C:\\Users\\Ana\\Documents\\Estágio\\Scripts\\')
...
# Constants
RAW_COUNTS_FOLDER <- 'C:\\Users\\Ana\\Documents\\Estágio\\Sun2023\\'
...
import_featureCounts <- function(counts_file, clinical_data, te_class = FALSE) {
  ...
  colnames(counts_df)[7:total_cols] <- sub("/mnt/comics-data/comics02/RTT_files/Sun2023/03_Uniquelly_Mapped/", "", colnames(counts_df)[7:total_cols])
  ...
  colnames(counts_df)[-1] <- clinical_data[["Sample_Name"]][match(srr_names, clinical_data$Run)] #Colocar na tabela final, todos os srr presentes na clinical data
...
dea_deseq <- function(counts_df, clinical_info, save_expression = TRUE, save_expression_name = "Expression.csv") {
  ...
  res <- results(dds, contrast = c("Condition", "MECP2_R270X", "WT"), alpha = 0.05)
  print(summary(res))
...
clinical_data <- read.csv(paste0(RAW_COUNTS_FOLDER, 'SraRunTable_Sun23.csv'))
